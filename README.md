...
`If` НЕЛЬЗЯ использовать без `else`.

Пример:
```sinenkiy
if a > b { val c = 6 } else { val c = 8 };
val a = 5;
val b = 5;
if a == b { val c = 6 } else { val c = 8 };
```
...
Функция `append` вставляет эллемент в начало списка. Сначала принимает элемент, потом список. 

Записывается как: `append(элемент, список);`
```sinenkiy
val list = append(9, [1, 2, 3, 4, 5]);
println(list);
```
Вывод:
